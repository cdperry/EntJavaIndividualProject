package com.cdperry.brewday.controller.types.UseType;

import com.cdperry.brewday.entity.UseTypeEntity;
import com.cdperry.brewday.persistence.UseTypeDao;

import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import java.io.IOException;

/**
 *  <p>
 *  This servlet is used to display the Add/Edit Use Type page
 *  </p>
 *  @author Chris Perry
 */
@WebServlet(
        name = "UseTypeAddEditDisplayServlet",
        urlPatterns = { "/addUseType", "/editUseType" }
)
public class UseTypeAddEditDisplayServlet extends HttpServlet {

    private UseTypeDao useTypeDao;

    public UseTypeAddEditDisplayServlet() {
        super();
        useTypeDao = new UseTypeDao();
    }

    /**
     *  This method handles HTTP GET requests.
     *
     *  @param  request                   the HttpServletRequest object
     *  @param  response                   the HttpServletResponse object
     *  @exception  ServletException  if there is a Servlet failure
     *  @exception  IOException       if there is an IO failure
     */
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {

        String url = "/jsp/editUseType.jsp";

        RequestDispatcher dispatcher
                = getServletContext().getRequestDispatcher(url);

        if (request.getParameter("action").equals("insert")) {
            request.setAttribute("actionType", "insert");
            dispatcher.forward(request, response);
        } else {
            int useTypeId = Integer.parseInt(request.getParameter("useTypeId"));
            UseTypeEntity useType = useTypeDao.getUseTypeEntity(useTypeId);
            if (useType != null) {
                request.setAttribute("useType", useType);
                request.setAttribute("actionType", "edit");
                dispatcher.forward(request, response);
            } else {
                url = "listUseTypes";
                response.sendRedirect(url);
            }
        }





    }

}
